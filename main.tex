...
% Most research on using motion-based depth estimation has been on reactive obstacle avoidance \cite{flame, avoidance_mono1} or exploration with strong assumptions on the environment structure (e.g. corridors only \cite{corridors_mono_nav_2009} or considering a single object with no other obstacles \cite{information_driven_bbx_mapping}).
...
% Since safety and flexible representation of both large-scale and narrow spaces are our main concerns, we decided that a graph-of-spheres representation \cite{spheregraph, spheremap, bubbleplanner} could be more suitable for representing free space than voxel grids or TSDFs \cite{octomap, voxblox, ufomap}.
Since safety and flexible representation of both large-scale and narrow spaces are our main concerns, we chose to use a graph-of-spheres representation \cite{spheregraph, spheremap, bubbleplanner} as the world model instead of voxel grids \cite{octomap, ufomap} or TSDFs \cite{voxblox} traditionally used in exploration literature .
...
% The proposed mapping method represents free space by a graph of intersecting spheres and obstacles as a set of 3D points.
...
\begin{figure}[!h]
  ...
  % The (dashed red) point seen far away from the camera at a previous time $t_1$ now lies in $\mathcal{P}_f^{t_2}$ very close to the camera and will thus be deleted as noise.
