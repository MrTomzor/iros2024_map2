...
\begin{enumerate}
    ...
    \item A novel occupancy mapping approach and representation -- an adaptation of SphereMap TODO-CITE, which does not require building an occupancy grid, and can be built using only odometry and sparse 3D point data from visual sensors as input
    \item A simple visual inverse-depth estimator module as a frontend for the proposed occupancy mapping, designed to be used if only image and odometry data are available on a system 
      % (such as when the odometry is obtained from GPS)
    \item Experimental verification that the proposed methods are well suited for safety-aware navigation and frontier-based exploration on a UAV equipped with only visual-inertial sensors in unstructured, unknown environments
...
In our previous work TODO-CITE-SPHEREMAP, we built a graph of spheres that describe the free space on top of Octomap \cite{octomap}.
...
However, in TODO-CITE-SPHEREMAP, we built the abstract representation \textit{on top} of Octomap \cite{octomap}, which takes considerable resources to update.
% In addition, 
...
As detailed in surveys such as TODO-CITE, constructing reliable vision-based autonomous UAV systems is still a challenge.
