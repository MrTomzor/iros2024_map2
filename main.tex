...
%
\todo{The existing monocular mapping approaches \cite{from_monoslam_to_explo, los_maps} use the same core approach which is used with dense range sensors: [NO! implicit!]}
...
Exploration of unknown environments is a well-studied research problem \cite{surveys_explo_nav_1, surveys_explo_nav_2, surveys_explo_nav_3} in the case of robots with dense range sensors, such as LiDARs or depth cameras.
...
In essence -- there is true occupied and free space, and the sensor 
"uncovers" all visible occupied and free space in the sensor's field-of-view and range at any given pose.
% can correctly classify all occupied obstacles within its field-of-view and range from any given pose.  
...
[TODO-monocular depth is still an open problem, largest issue is GENERALIZATION - cite surveys.]
[TODO-depth from motion using SLAM requires TEXTURE, and is sparse even when dense. + this adds a TRANSLATION to map space, while dense sensors dont require motion to get depth.]
...
Thus, it is not possible to directly apply the same mapping and planning approaches as with dense range sensors and the existing exploration approaches either fail to explore large-scale environments or make limiting assumptions on the environment structure.
Existing approaches to solving this problem can be broadly divided into three main categories: 
...
\todo{navigation vs exploration, also STRUCTURE-ASSUMPTION methods}
...
% To simplify the method and reduce memory usage, we do not store the position covariance for each point, but rather we store the minimum distance from which any point $\mathbf{x}$ has been observed, denoted further as $d_{m, x}$.
...
%       \node[anchor=south west,inner sep=0] (a) at (0,0)
%         {\adjincludegraphics[width=1.0\linewidth,
%           % trim={{0.27\width} {0.05\height} {0.15\width} {0.07\height}},
%           trim={{0.3\width} {0.08\height} {0.15\width} {0.2\height}},
%           clip=true]{fig/orchard_brightened.png}};
%       % label (a) in lower-left corner
%       % \node[anchor=south west, font=\bfseries, text=black, xshift=2pt, yshift=2pt] at (a.south west) {(a)};
...
%       \node[anchor=south west,inner sep=0] (image) at (0,0)
%         {\adjincludegraphics[width=1.0\linewidth,
%           % trim={{0.09\width} {0.05\height} {0.05\width} {0.13\height}},
%           % clip=true]{fig/2025-10-20_17-28_orchard_ortho2.png}};
%           trim={{0.1\width} {0.06\height} {0.1\width} {0.13\height}},
%           clip=true]{fig/2025-10-20_17-20_orchard_treesvisible.png}};
%           % clip=true]{fig/2025-10-20_17-31_orchard_3d_v2.png}};
%           % clip=true]{fig/2025-10-20_17-29_orchard_3d.png}};
%           % clip=true]{fig/2025-10-20_17-28_orchard_ortho2.png}};
%           % clip=true]{fig/2025-10-20_17-25_orchard_260_ortho1.png}};
%           % trim={{0.1\width} {0.03\height} {0.15\width} {0.065\height}},
%           % clip=true]{fig/2025-10-20_15-21_orchard_rviz_topdown2.png}};
%           % trim={{0.00\width} {0.00\height} {0.0\width} {0.0\height}},
%           % clip=true]{fig/2025-10-20_16-14_boxes_poses_topview_blue2.png}};
%       % label (b) in lower-left corner
%     \begin{scope}[x={(image.south east)}, y={(image.north west)}]
%       % FOR TREEVISIBLE
%         \draw[blue, very thick] (0.45,0.35) circle (10pt);
...
%      \hfill
%   \caption{
%     The 4min orchard exploration experiment.
%     }
%   \label{fig:orchard}
% \end{figure}
% %%}
...
\centering
 ...
 \hline\hline
    ...
    No DBOF  & Urban & 5118 $m^2$ & 5425 $m^2$ & 5 \\ 
    No OVDE  & Urban & 5118 $m^2$ & 5425 $m^2$ & 5 \\ 
    No VPB  & Urban & 5118 $m^2$ & 5425 $m^2$ & 5 \\ 
    \hline
