...
\begin{abstract}
  ...
  We present a novel spatial mapping approach that enables UAVs to explore large-scale 3D environments and requires only sparse pointclouds and pose estimates generated by monocular-inertial SLAM as input. Instead of traditionally casting rays of free space towards obstacle points to build an occupancy grid-based representation, our method inscribes free-space spheres inside a polyhedron constructed around the camera's position and the currently visible triangulated visual keypoints at each iteration. Intersecting spheres then form a topological graph suitable for rapid and safety-aware planning. We also present a scheme for safely sampling additional free space in directions where no textured points can be triangulated, and show how this enables exploring outdoor open-space areas. Furthermore, we present a perception-aware exploration approach that is enabled by the proposed mapping method and explicitly handles the problematic properties of monocular depth estimation. The proposed methods are, to the authors' best knowledge, the first to achieve autonomous exploration in real-world, unknown, unstructured 3D environments at the presented scale, using only a single monocular camera and an IMU onboard a potentially micro-scale UAV. We open-source the code for the proposed methods to provide safety-aware 3D navigation capabilities to any vision-based UAV operating in unknown environments.
...
These advances are critical for allowing the deployment of uncrewed aerial vehicles (UAVs) in search-and-rescue and planetary exploration missions.
...
Other methods \cite{simon2023mononav} use depth neural networks to obtain denser pointclouds, 
...
In several experiments, we show how this mapping approach, combined with perception-aware planning, enables exploration large-scale unstructured areas which feature both textureless wide open spaces and cluttered narrow passages.
% TODO - is neccesary?
...
% For monocular exploration in unstructured environments, the authors of \cite{from_monoslam_to_explo, los_maps} proposed taking the points from monocular SLAM, treating them as sparse distance measurements, and building grid-based maps in the same way as with dense pointclouds.
For full monocular exploration in unstructured environments, the authors of \cite{from_monoslam_to_explo, los_maps} proposed building grid-based maps by tracing rays of free space towards the monocular SLAM points. 
...
The authors of \cite{los_maps} also trace rays towards sparse SLAM points, and present exploration on the scale of several indoor rooms and in simulation only.
...
Grid-based maps built by raytracing suffer from the problem of choosing the smallest voxel resolution in multi-scale environments.
...
% \begin{itemize}
  Firstly, the methods presented in \cite{spheremap, spheregraph, bubbleplanner} require building a local occupancy grid with a LiDAR for computing the sphere-based map.
    % \todo{This can be computationally expensive + there has been now approach shown to build GRID using sparse depth data, making it unsuitable for large featureless environments?}
    This can be computationally expensive.
    Secondly, all methods for building a grid-based occupancy map require the data to be from dense depth sensors. 
...
This assumption allows estimating more free space than when only tracing rays towards the visual SLAM points localized in 3D.
...
However, it does not hold close to large textureless objects or thin objects where no features can measured by the sparse visual SLAM frontend.
...
This is designed to solve problems that can arise when viewing some surface from a much higher distance than before (e.g. observing a wall from 5 m, and then later from 20m).
...
In the following sections, we introduce several major differences in methodology that are critical for allowing the NBV strategy in the case of a robot equipped with only a monocular camera for depth sensing.
...
Even with this unoptimized implementation, the mapping keeps below $\SI{1.5}{\second}$ per map update on a standard CPU, making it suitable for real-world deployment.
...
In \autoref{fig:exper_real}, we present a 20min experiment where our approach is tested in real-world conditions.
...
This was made possible by our approach to safe estimation of free space in open-space areas, described in \autoref{sec:fake}.
...
We have tested the proposed approach extensively in a variety of other real-world environments, including a forest path, a tree in a wide open field, and a campsite, which our method also explored.
Videos from real-world experiments are available at TODO-YOUTUBE-LINK. 
...
\begin{figure}[htb]
  ...
  \centering
     ...
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0\height}}, clip=true]{fig/fireworld_max_octomap.png}};
      \end{tikzpicture}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0\height}}, clip=true]{fig/fireworld_max_mono.png}};
      \end{tikzpicture}
     \end{subfigure}
     \hfill
  \centering
  \caption{Best-result maps of the Grid-based exploration approach (bottom left) and MonoSpheres (bottom right) in the urban exploration experiments, and the exploration progress of all the runs (top left). Green = MonoSpheres, Blue = Grid-based.
  }
...
\begin{figure}[htb]
  ...
  \centering
     ...
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0\height}}, clip=true]{fig/cave_astar_final.png}};
      \end{tikzpicture}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0\height}}, clip=true]{fig/cave_spheremap_final.png}};
      \end{tikzpicture}
     \end{subfigure}
     \hfill
  \centering
  \caption{Best-result maps of the Grid-based exploration approach (bottom left) and MonoSpheres (bottom right) in the cave exploration experiments, and the exploration progress of all the runs (top left). Green = MonoSpheres, Blue = Grid-based. 
  }
...
\centering
 ...
 \hline\hline
    ...
    \hline
    MonoSpheres  & Cave & 5937 $m^2$ & 6012 $m^2$ & 0/2 \\ 
    Grid-Based & Cave  & 3331 $m^2$ & 3375 $m^2$ & 0/2 \\
 \hline
\end{tabular}
  \caption{Quantitative results of the simulation experiments described in \autoref{sec:sim_exper}. TODO - run more cave experiments}
...
Since the existing monocular exploration methods do not have their code published (with the exception of \cite{simon2023mononav}, which, however, is not ROS-compatible as of writing this paper), we prepared a simple grid-based mapping and exploration pipeline for comparison.
This reference grid-based method uses the same mapping approach as \cite{from_monoslam_to_explo, los_maps} (i.e. initializing free space between the camera and the 3D points localized by visual SLAM). 
To explore, the method periodically samples free-space points uniformly in a 40x40x10 area around the robot and sends these as goals to a grid-based A* path planning module, which finds path through the occupancy grid that would move the robot as close as possible to the goal while respecting the same distance from obstacles and uknown space as the MonoSpheres method. The robot then follows these paths with the camera pointing forward and replans at 5Hz to handle measurement inacurracies. 
We use OctoMap for the grid-based mapping with a \SI{0.5}{meter} cell size.
This value was chosen because smaller cell sizes caused the robot to not find any safe paths due to the free-space gaps documented in \cite{from_monoslam_to_explo} and larger cell sizes blocked exploration of tight corridors. 

As the evaluation metric, exploration literature usually uses the volume of the constructed map. However, because of the open-space free-space sampling scheme and the depth interpolation, MonoSpheres constructs vastly more free space than the raytracing mapping approach over the same trajectories. For this reason, this metric would not be fair towards methods using the grid-based raytracing mapping. 
...
Exploration goal generation was constrained to a \SI{60x50x7}{meter} area of interest in the urban world for both methods, and was unconstrained in the enclosed indoor world.
Videos from these experiments are available in the multimedia materials.

\subsection{Simulation Experiments Discussion}
...
These results demonstrate that our proposed open-space free-space sampling approach described in \autoref{sec:fake} enables exploration of outdoor areas where the ray-tracing grid-based mapping is not able to create sufficient free space, while safely mapping even indoor areas.
...
In all of the experiments, our method achieved long periods of collision-free flight even near complex obstacles such as trees, collapsed houses, and cars in uncommon configurations.
The few documented crashes were caused by either occasionally incorrect free-space sampling \autoref{sec:fake}, or by small featureless obstacles (such as blackened parts of the simulated burned buildings or exposed metal rods) that were not detected as features by point-based visual SLAM frontend.
...
These issues would be solved by leveraging low-cost ultrasound sensors as a bumper for the UAV.
Another possibility of detecting these object would be to use a visual front-end that uses line-based or dense features, but that is beyond the scope of this article.
