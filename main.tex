...
The existing monocular mapping approaches use the same core approach which is used with dense range sensors:
...
Some methods \cite{from_monoslam_to_explo, los_maps} cast rays towards sparse pointclouds given by monocular SLAM, which leads to large gaps in free space, prohibiting planning in texture-sparse areas.
...
The main idea of our paper is that instead of traditionally casting rays through a grid towards obstacle points and updating occupancy values of passed voxels \cite{octomap, voxblox}[\todo{MORE CITATIONS}], we construct a polyhedron of approximate visible free space \todo{at each frame} and inscribe spheres of free space inside that polyhedron.
Intersecting spheres are connected together to form a graph-of-spheres map representation that has been shown to allow rapid and safety-aware planning \cite{spheremap} \todo{MORE CITATIONS}.
...
Combined with the sampling scheme described in \autoref{sec:fake}, this approach accurately estimates free space even in low-texture areas where the traditional raytracing approach might not initialize sufficient free space to allow flight, as shown in our experimental results in \autoref{sec:sim_exper}. 
