...
\begin{figure*}[!htb]
  ...
  \caption{
    ...
    (a) The stable localized points (green) used by the monocular SLAM are the only input distance measurements. 
    (b) Delaunay triangulation is performed on the input points projected to 2D as a rough depth interpolation. 
    % Additionally, , the TODO-fake points are added to the depth estimation.
    Additionally the active assumed-freespace points (see \autoref{sec:fake}) above the ground, where there is no depth information, are added to construct the
...
Additionally, we plan to use the proposed mapping approach for building submaps instead of a single large map and performing large-scale map optimization to achieve low-cost autonomy at the scale of kilometers, which has only been demonstrated with LiDAR-based systems.
