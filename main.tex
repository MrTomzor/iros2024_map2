...
Most importantly though, this mapping approach cannot \todo{fundamentally?} estimate free space in textureless areas, especially outdoor (as shown in \autoref{sec:sim_exper}).
...
In this paper, we use the sparse point-based \todo{OpenVINS} \cite{openvins} method as the SLAM frontend, but our method could work with any other monocular-inertial SLAM.
...
Then, if the UAV's camera could observe sufficient parallax for estimating the distance of $\mathbf{x}_{vir}$ between any points on the trajectory between $t$ and $t-\tau$, and if the visual SLAM frontend has not estimated any stable 3D point near  $\mathbf{x}_{vir}$, then $\mathbf{x}_{vir}$ must lie in free space.
...
\end{equation}
where $\mathbf{p}_{cam}$ is the position of the camera and $\alpha$ is a paremeter, which is usually set to $1.1$ to account for distance measurements uncertainty.
...
With this implementation, the mapping runs at $\SI{2}{\hertz}$ on average on a standard CPU, which is suitable for real-time operation, and it could be further optimized with a parallelized implementation or a submap-based approach.
...
For state estimation and as the source of sparse visual SLAM points, we used \todo{OpenVINS} \cite{openvins} in the inverse-depth measurement mode.
