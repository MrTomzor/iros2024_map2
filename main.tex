...
\begin{abstract}
  ...
  In this paper, we present a novel approach to monocular vision-based exploration that can \todo{safely} cover large-scale unstructured indoor and outdoor environments by explicitly accounting for the properties of a sparse monocular SLAM frontend in both mapping and planning.
...
% Due to the high uncertainty and sparsity of depth estimates, 
% The graph-of-spheres representation was chosen due to its scalable free-space representation and rapid path-planning capabilities \cite{spheregraph, spheremap, bubbleplanner}. 
% The graph-of-spheres representation was chosen due to its scalable free-space representation and rapid path-planning capabilities \cite{spheregraph, spheremap, bubbleplanner}. 
...
% In this paper, we present an approach to monocular exploration that works by explicitly taking into account the properties of sparse monocular SLAM in not only the planning, but also the \textit{mapping}.
...
\end{minipage}
  ...
  \centering
  \caption{Trajectories of the best MonoSpheres (black) and Grid-Based explorer (blue) simulated experiments per each world (top) along with the resulting MonoSpheres mapped obstacle points (bottom) colored by height. X marks the starting position.
  }
