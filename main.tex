...
\end{minipage}
  ...
  \caption{Illustration of the proposed approach. The mapping pipeline \fix{estimates depth using virtual (blue) and tracked (red) points} from sparse monocular-inertial SLAM (left) running on board the UAV (top) to construct a large-scale map (right) consisting of free-space spheres (green), obstacle points (red) \fix{and frontier points (magenta).}}
...
\begin{figure}[!h]
  ...
  The visible free-space polyhedron $\mathcal{P}_f^{t_2}$ is created using the \fix{low-covariance SLAM points tracked at $t_2$} (black crosses) and the camera's \fix{pose} $\mathbf{p}_{cam}^{t_2}$.
  The size of a point $\mathbf{x}$ \fix{corresponds} to the distance $d_{\mathbf{x}}$ that it was measured from.
  \fix{Since our method prioritizes closer measurements (of obstacles and lack thereof), only the (dashed red) point seen far away from the camera at a previous time $t_1$, which now lies in $\mathcal{P}_f^{t_2}$ very close to the camera, will be deleted as noise.}
  New spheres are inscribed inside $\mathcal{P}_f^{t_2}$, but their radii are also bound by the protected points (solid red) $\mathcal{X}_p$, which \fix{were added to the map at $t_1$ and not deleted at $t_2$ even though they lie in $\mathcal{P}_f^{t_2}$.
 ...
 \def\subfigwidth{0.48\linewidth}
  ...
  \subfloat{%
    ...
    \end{minipage}
     ...
     \hfill
  \caption{
    \fix{Top-down illustration of the exploration approach.
...
\centering
  ...
  \caption{
    \fix{The 4min orchard exploration experiment. Right: explored map (approx. 40x30m wide), the UAV's trajectory (small black arrows) and trees (circled blue).}
  }
  \label{fig:orchard}
\end{figure}
% %%}
...
\end{minipage}
  ...
  \centering
  \caption{\fix{Trajectories of the best MonoSpheres (black) and Grid-Based explorer (blue) simulation experiments per each of the 4 world (top) along with the resulting MonoSpheres mapped obstacle points (bottom) colored by height. X marks the starting position.}
  }
...
\end{minipage}
  \centering
  \caption{\fix{Ablation test results on the earthquake world. Left: visualization of exploration progress, best runs per method are thick and a marker signifies a crash or mission completion. Right: best-run trajectories for each method.}
  }
  \label{fig:ablations}
\end{figure}
...
% %%{
\begin{table}[h!]
  \caption{\fix{Quantitative results showing the explored area in the simulation experiments described in \autoref{sec:sim_exper}.} }
\centering
