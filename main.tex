...
The main question addressed in this paper is -- "How to construct such a representation using only a single camera and inexpensive metric-scale sensors, such as an inertial-measurement-unit?"
...
However, to build an occupancy grid, one needs to have dense pointclouds of range measurements due to the nature of occupancy grid updates - which is to cast rays towards the measurement points and update the occupancy of traversed cells.
% TODO - check if all
...
However, being able to build a representation that would allow safe and rapid exploration and navigation on-board UAVs with such inexpensive sensors would economically unlock many potential applications, such as swarms of  UAVs for search and rescue missions.
In addition, RGB cameras are also useful for object recognition or other types of vision-based artificial intelligence, and thus it makes sense to have them on many UAVs by default.
Furthermore, when moving, a single camera can also provide rough estimates on very distant objects distance, far beyond the \~50m range of LiDARs, but this advantage has not yet been fully utilized in existing research.
...
In this paper, we present a novel approach to occupancy mapping that takes in a sparse pointcloud of tracked visual keypoints (already available if using some visual/visual-inertial odometry), and instead of traditionally raycasting and updating cells in a grid, we construct a polyhedron of approximate visible free space at each frame, inscribe spheres of free space inside that polyhedron, and connect them together to form a graph of intersecting spheres that allows for fast and safety-aware planning.
...
In this paper, we represent the free space using spheres as well, but with two major differences that now \textit{we do not need to build the occupancy grid as an intermediate step}, which frees up significant computational resources, and \textit{we construct it from sparse point-clouds from monocular SLAM}, which are problematic for occupancy grids.
...
\begin{enumerate}
  ...
  % \item A novel occupancy mapping approach, which constructs a sphere-based occupancy representation that offers the same information as an occupancy grid, but is more suitable for path planning, using odometry and sparse pointclouds obtained from monocular SLAM.
    ...
    \item Open-sourced code for the novel occupancy mapping method, along with path-planning and exploration functionalities and example scripts for effortlessly enabling affordable UAV autonomy.
...
We propose to represent free space by a graph of intersecting spheres, similarly to our previous work \cite{spheremap}, and obstacles by points corresponding to triangulated visual keypoints from monocular SLAM.
...
In this section, we explain how such map can be built directly from the outputs of monocular SLAM, without needing any intermediate occupancy grid computation, and how the sparsity and high depth uncertainty of the input pointclouds is handled. 
...
In the experiments in this paper, we obtain the pointclouds and pose estimates from OpenVINS \cite{openvins}, using its mode of estimating inverse depth estimates.
...
There can be object that are too small to be detected by the visual SLAM at large distnaces, so the free-space estimates might be wrong, but we deal with that in Sec \ref{sec:distance-based}.

Our method can, in principle, work with pointclouds from any source, even dense sensors such as depth cameras or LiDARs, and representing everything as elements in space allows storing information about e.g. some space looking as free using camera data, but occupied according to a depth camera (such as in a window).
...
There, the authors focus on constructing a precise mesh from visual keypoint measurements, but here, we care primarily about mapping the free space, for planning purposes, and thus we do not perform the mesh optimization or splitting the 2D interpolations done in \cite{flame}.
...
We enclose this space by connecting all points that lie on the edge of $F_o$ to the camera's focal point, which forms an \textit{estimated visible free space polyhedron} $P_f$, visualized in Fig.\ref{fig:smap_intro}.
...
\begin{equation}
  ...
  \label{eq:update}
\end{equation}
where $d(x, P_f)$ is the signed distance to $P_f$ (positive if the point is inside the polyhedron, negative if outside) and $d(x,X \cup X_p)$ is the minimum distance to all input obstacle points $X$, and to obstacle points $X_p$ which lie in the map, fall into $P_f$, but are not deleted in the current frame, as described more in Sec. \ref{sec:distance-based-points}.
...
\label{sec:distance-based-points}
...
We solve both these problems by deleting map point $x$ that falls into $P_f$ only if
\begin{equation}
