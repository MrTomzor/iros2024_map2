...
These advances can enable deployment of UAVs in real-world, unknown environments, for example in urban areas after an earthquake, a forest during a wildfire, or caves on other planets.
...
However, the mapping and exploration methods used for large-scale autonomy currently rely on dense range sensors, such as LiDARs or depth cameras.
% , for building 3D maps for exploration and path planning.
Such sensors are heavy, expensive, and thus severely limit the affordability and flight time of existing autonomous UAVs.
...
Enabling UAVs to autonomously map and navigate unknown environments using such sensors would significantly lower the cost of many real-world UAV applications.
...
Some works have demonstrated limited outdoor exploration, but only with strong constraints on the environmental structure
\cite{avoidance_mono1, corridors_mono_nav_2009}.
...
In this paper, we present a novel approach to 3D mapping of unstructured large-scale environments using only a monocular camera and IMU. 
...
Instead of traditionally casting rays through a grid and updating occupancy values of voxels \cite{octomap, voxblox}, we construct a polyhedron of approximate visible free space at each frame, inscribe spheres of free space inside that polyhedron, and connect them together to form a graph of intersecting spheres.
\todo{Under some assumptions (see \autoref{sec:fake}), which hold in most outdoor real-world areas, this allows large-scale exploration...}
...
This allows for rule-based measurement fusion, which is critical for dealing with the challenges of monocular depth estimation, as we explain in \autoref{sec:distance-based}.
In several experiments, we show how this mapping approach, combined with perception-aware planning, enables exploration in outdoor, large-scale areas which feature both textureless wide open spaces and cluttered narrow passages.
...
\begin{itemize}
  ...
  \item In \cite{spheremap, spheregraph, bubbleplanner}, the method required building a local occupancy grid with a LiDAR for updating the spheres. 
    In this paper, we build the graph of spheres \textit{directly} out of input measurement points without building any intermediate occupancy grid, and only require a monocular camera running visual SLAM instead of a LiDAR.
...
% TODO-CHECK
  \item Additionally, the representation proposed in this paper stores previously seen SLAM points as obstacle points and remembers their measurement distance. 
...
This assumption allows estimating more free space than when only raycasting towards observed SLAM points.
...
\section{CONCLUSION}
In this paper, we proposed a novel approach to building a 3D map for navigation using a graph of spheres instead of a traditionally used voxel grid as the base representation, and presented how such a map enables safety-aware, large-scale exploration. 
Our method only requires a monocular camera and an IMU, and can run on a standard onboard computer without needing a GPU.
Compared to the existing approaches to 3D monocular mapping and exploration, which have so far presented autonomy only at the scale of a few indoor rooms \cite{from_monoslam_to_explo, los_maps,deep_mono_depth_2021, deep_mono_depth_2019}, our proposed approach enables autonomous exploration at the scale of an urban area with multiple buildings, trees and wide open spaces, with all of the computations running on-board the UAV.


The proposed methods open the way for micro-scale UAVs operating fully autonomously in unknown indoor/outdoor large-scale environments with obstacles, but there are still many challenges to be solved.
