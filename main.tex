...
In comparison to the existing sphere-based mapping methods however, our proposed approach is the first to build the map \textit{directly} from sparse monocular depth data, as the others require a dense occupancy voxel-based map to be built to extract the graph of spheres. 
...
To represent obstacles, we accumulate the stable 3D points (i.e. with low position covariance) estimated by the monocular SLAM into a set of points $\mathcal{X}$ with a minimal point-to-point distance $\xi$ (anlogous to voxel size in voxel-based maps).
