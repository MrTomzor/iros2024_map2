...
\begin{enumerate}
  ...
  \item A novel occupancy mapping method which constructs a sphere-based occupancy representation from sparse 3D point data obtained from monocular SLAM, and odometry as input, easily extendable with dense point cloud data from depth sensors as well.
    ...
    \item A demonstration of large-scale 3D outdoor exploration on a UAV equipped with only monocular-inertial sensors, enabled by the proposed occupancy mapping method, along with important changes to traditional exploration approaches necessary for when using a monocular camera, experimentally verified in simulation and in the real world.
      % on a UAV equipped with only a single camera and IMU as primary sensors, in simulation and in the real world.
    \item Open-sourced code for the novel occupancy mapping, along with path-planning and exploration functionalities and example scripts for effortlessly enabling affordable UAV autonomy.
...
Our method works best when using inverse-depth parametrization [INV] of point locations instead of estimating full 3D positions of visual keypoints in the SLAM.
...
The input of a single map update is odometry and a sparse 3D pointcloud, 
...
\subsection{Handling Depth Estimation Errors over Large Distances}
