...
\begin{abstract}
  ...
  We demonstrate how our method can be used to, to the best of our knowledge, achieve the first ever autonomous UAV exploration in unstructured 3D environments, using only a single camera and IMU as primary sensors.
  ...
  We open-source the code for our approach to enable occupancy mapping and navigation on any UAV with a camera and a source of odometry.
...
However, the vast majority of autonomous navigation research in the real world has been focused on robots equipped with costly sensors that provide dense depth data, such as LiDARs \cite{beneath}, depth cameras [TODO] or stereo camera pairs \cite{explor_stereo}.
...
Even though single vision sensors are much cheaper, their use for online map building and autonomous navigation on UAVs is still severely limited.
Most such approaches in vision-based robotics are reactive behaviors \cite{avoidance_mono1}, or make strong assumptions about environmental structure \cite{corridors_mono_nav_2009}.

TODO - about sparse pointclouds from mono cameras, how not used, how invdepth also not used much for occupancy

To move towards enabling vision-based autonomous navigation in unknown, unstructured 3D environments for UAVs, our paper offers the following contributions:
\begin{enumerate}
...
\section{Direct Sphere-Based Occupancy Mapping}
...
\subsection{Simulated Large-Scale Exploration}
...
for sparse point cloud data from monocular visual sensors,
