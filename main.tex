...
With only a single camera however, the task is even more challenging, as obtaining correct depth estimates is harder than with stereo cameras.
...
Another approach to single-camera autonomy would be to use deep-learning methods for monocular depth estimation [TODO] and then using well-researched methods for mapping, exploration and planning as on systems with a depth camera.
...
Furthermore, this approach requires having a GPU on-board the UAV, which can strongly increase the system cost, and reduce flight time due to weight and power requirements.
