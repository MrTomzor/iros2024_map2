...
% The problem addressed in this paper is an instance of volumetric exploration problem.
...
In the case of robots equipped with classic dense depth sensors (most commonly LiDAR, RGBD, stereo cameras), this is a well-studied research problem \cite{surveys_explo_nav_1, surveys_explo_nav_2, surveys_explo_nav_3}.
...
These works implicitly assume that from any given viewpoint, the sensor will uncover all obstacles up to its sensing range, and that non-detection of obstacles almost always means that free-space can be initialized in the given direction  \cite{occupancy_moravec, octomap, voxblox, ufomap}.
...
We base our solution to this problem on the following key assumptions/observations about monocular motion-based depth sensing:
\begin{enumerate}
  \item Depth accuracy generally increases with decreasing distance to obstacles, as the \textit{inverse} of depth errors are gaussian \cite{inverse_depth} and since the objects occupy more pixels in the image, making them more likely to be detected.
    \item In outdoor environments, there can be large textureless areas where no depth can be estimated (e.g. above horizon on a clear day) by classic motion-based methods that initialize depth by tracking texture.
    \item Measuring depth requires sufficient translational motion, making the depth estimates \textit{trajectory dependent}, which is not the case with dense range sensors.
\end{enumerate}
...
\begin{figure*}[htb]
    ...
    % ROW 2
     ...
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        ...
        % HEIGHTMAP
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0.0\height}}, clip=true]{fig/monoexplo_bigmaps/2025-10-24_16-28_cave_heightmapscale4.png}};
      \end{tikzpicture}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{\subfigwidth}
     ...
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        ...
        % \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0.0\height}}, clip=true]{fig/monoexplo_bigmaps/2025-10-24_15-45_sparse_range2_05squares.png}};
        % THICK HEIGHTMAP
  ...
  \centering
  \caption{Top simulated runs of MonoSpheres with MonoSpheres (black) and Grid-Based explorer (blue) trajectories
  }
...
\begin{table}[h!]
\centering
  \begin{tabular}{m{18mm} m{10mm} m{10mm} m{10mm} m{10mm}} 
 \hline
    % \vspace{0.1mm}
 ...
 \hline\hline
    ...
    MonoSpheres  & Urban & 1968& 2432& 17706 \\
    Grid-Based & Urban  & 764& 922& 5181 \\
    No FFF  & Urban & 988& 1272& 9518 \\
    No DBOF  & Urban & 923& 1232& 10356 \\
    No OVDE  & Urban & 299& 345& 1837 \\
    No VPB  & Urban & 1479& 1670& 13475 \\
    \hline
...
To determine the contribution of the individual components of our proposed mapping and exploration method, we performed ablation tests in the urban world with 3 exploration runs for each ablation.
...
Without DBOF, the UAV tends to quickly delete obstacles and crash into them.
Without FFF, the UAV is unable to detect previously undetected obstacles and also crashes fast.
