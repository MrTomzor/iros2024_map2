...
Vision-based UAV autonomous exploration has so far, by far not reached the levels of autonomy using range sensors, and remains a challenge, as documented in recent surveys \cite{drones_survey}, but some progress has been made.
...
The authors used the dense depth pointcloud coming from the stereo cameras for building an OctoMap \cite{octomap} occupancy representation in the same way as with depth data from an RGB-D camera or LiDAR.
