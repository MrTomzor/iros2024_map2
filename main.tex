...
MonoSpheres: Large-Scale Monocular UAV Exploration using Perception-Coupled Mapping and Planning}
...
\begin{abstract}
  ...
  This is primarily caused by the difficulties of building reliable and complete 3D maps with unreliable and sparse monocular depth data.
  %
  % Autonomous exploration of unknown environments is a key capability for mobile robots.
  % It is however largely unsolved for robots equipped with a single monocular camera, primarily because of the difficulties of building reliable and complete 3D maps with unreliable and sparse monocular depth data.
  %
  % \fix{We present a novel approach to monocular vision-based exploration that can safely map unknown large-scale, texture-sparse indoor and outdoor environments thanks to tight coupling between perception, mapping and action planning.} 
  % \fix{In this paper, we present a novel approach to monocular vision-based exploration that can safely map unknown large-scale, texture-sparse indoor and outdoor environments thanks to tight coupling between perception, mapping and action planning.} 
  % In this paper, we present a novel approach to monocular vision-based exploration that explicitly handles the properties of monocular depth estimation both in the mapping and planning modules. 
  % Our method inscribes free-space spheres inside a polyhedron constructed around the camera's position and the visible SLAM points with low position covariance, mapping more free space than traditional ray-casting methods.
  % Intersecting spheres form a topometric map suitable for rapid safety-aware planning. 
  In this paper, we present a novel approach to monocular vision-based exploration that can safely cover large-scale unstructured environments by explicitly accounting for the properties of a sparse monocular SLAM frontend.
  % We redesign classic mapping and planning approaches to explicitly address the problematic properties of monocular depth estimation.
  The mapping module solves the problems of sparse depth data, free-space gaps, and large depth uncertainty by oversampling free space in texture-sparse areas and keeping track of obstacle measurement distances.
  The navigation module handles the added free-space uncertainty through rapid replanning and forced forward flight.
  ...
  % We develop a scheme for safely sampling additional free space in textureless areas where depth estimation is challenging, and show how this enables navigation in outdoor open-space areas. 
  % We present a perception-aware exploration approach that is enabled by the proposed mapping approach and explicitly handles the problematic properties of monocular depth estimation. 
...
%
The existing monocular mapping approaches \cite{from_monoslam_to_explo, los_maps} use the same core approach which is used with dense range sensors:
...
Some methods \cite{from_monoslam_to_explo, los_maps} trace rays towards sparse pointclouds generated by monocular SLAM, which leads to large gaps in free space, prohibiting planning in texture-sparse areas.
...
\todo{add sensor-specific maps, reorganize, cut}
...
In the field of robotic exploration, a grid-based map of occupied, unknown, and free space is most often used as the robot's primary world representation, either in the form of a signed distance field \cite{voxblox} or an occupancy grid \cite{occupancy_moravec, octomap}.
...
% \todo{However, these representations rely on dense distance sensors, such as LiDARs or depth cameras, to build an accurate map of free and occupied space.}
However, these methods often implicitly rely on the map being built with sensors that produce dense distance measurements, such as LiDARs or depth cameras.
% However, methods using these representations implicitly assume that the map is built with dense distance sensors, such as LiDARs or depth cameras.
% However, these representations are designed to be used with dense distance sensors, such as LiDARs or depth cameras.
...
If the minimum voxel size is too small, gaps can appear in free space due to rays passing only a thin strip of voxels, especially with sparse depth data.
...
Our proposed mapping approach represents free space by a graph of intersecting spheres.
...
To represent obstacles, we accumulate the stable 3D points (i.e. with low position covariance) estimated by the monocular SLAM into a set of points $\mathcal{X}$ with user-defined minimal point-to-point distance.
...
In this paper, we use the sparse point-based OpenVINS \cite{openvins} method as the SLAM frontend, but our method could work with any other monocular-inertial SLAM.
...
The first step is to construct a polyhedron of space that is likely to contain free space based on the information in the current timestep.
...
Consider a virtual point in space $\mathbf{x}_{vir}$ at time $t$ that has not left the camera's FoV from a given time $t-\tau$ in the past up to $t$ 
(i.e. it could have been tracked by visual SLAM).
% (i.e. it has not left the camera's FOV between $t-x$ and $t$).
Then, if the UAV's camera could have observed sufficient parallax for estimating the distance of $\mathbf{x}_{vir}$ between any two points on the trajectory between $t$ and $t-\tau$, and if the visual SLAM frontend has not localized any stable 3D point approximately in the direction of  $\mathbf{x}_{vir}$, then $\mathbf{x}_{vir}$ must lie in free space.

The mapping method periodically checks 
the described condition for a set of virtual points $\mathbf{x}_{vir}$ at fixed positions relative to the UAV (see \autoref{fig:mapping_pipeline}) at each mapping iteration. 
...
Adding these assumed free-space points into the construction of the depth mesh allows estimating significantly more free space than when only tracing rays towards the visual SLAM points localized in 3D.
However, the assumption does not hold close to objects that cannot be detected by the used visual SLAM frontend.
Therefore, free space can sometimes be wrongly estimated when such objects appear, as we further discuss in \autoref{sec:discussion}.
...
\begin{figure}[!h]
  ...
  The size of a point $\mathbf{x}$ inversely corresponds to the distance $d_{\mathbf{x}}$ that it was measured from.
...
An important part of the proposed mapping method is that we store the minimum distance from which any point $\mathbf{x}$ has been observed, denoted further as $d_{m, x}$, as a very rough approximate of its position covariance.
...
As illustrated in \autoref{fig:distbased}, points corresonding to a small obstacle might momentarily not be detected by the visual frotend, while a larger obstacle behind the small one is detected.
...
We propose to sample points along the visible free-space polyhedron described in \autoref{sec:map_building} at each map update, and add the points as frontiers, if they do not lie inside any free-space sphere and if they are at a user-defined distance from all map obstacle points and other frontiers.
...
The presented implementation of the proposed mapping and exploration methods is \fix{currently} single-threaded and written in python.
With this implementation, the mapping runs on average at $\SI{4.5}{\hertz}$ on average on a standard CPU.
\fix{This is sufficient for exploration planning, but can be slow to detect obstacles at high speeds. For this reason, the UAV also checks the predicted trajectory for potential collisions with the input pointclouds at a higher rate.}
  ...
  % ROW 1
    ...
    % ROW 3
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0.0\height}}, clip=true]{fig/fireworld_monospheres_final.png}};
      \end{tikzpicture}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0.0\height}}, clip=true]{fig/monoexplo_cave_mono_final2.png}};
      \end{tikzpicture}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0\height}}, clip=true]{fig/Selection_1323_rooftops_mono_40min.png}};
      \end{tikzpicture}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0\height}}, clip=true]{fig/Selection_1318_sparse_mono.png}};
      \end{tikzpicture}
     \end{subfigure}
     ...
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0.0\height}}, clip=true]{fig/fireworld_astar_final.png}};
      \end{tikzpicture}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0.0\height}}, clip=true]{fig/monoexplo_cave_astar_final.png}};
      \end{tikzpicture}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0\height}}, clip=true]{fig/Selection_1324_rooftops_astar.png}};
      \end{tikzpicture}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0\height}}, clip=true]{fig/Selection_1320_sparse_astar.png}};
      \end{tikzpicture}
     \end{subfigure}


  \centering
  \caption{Best-result maps of the Grid-based exploration approach (bottom row) and MonoSpheres (middle row) in the simulation experiments.
  }
...
\begin{figure}[htb]
  ...
  \centering
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0.0\height}}, clip=true]{fig/ablations2.png}};
      \end{tikzpicture}
     \end{subfigure}
  \centering
...
\centering
 ...
 \hline\hline
    ...
    ABL-DistBased  & 5 & 5118 $m^2$ & 5425 $m^2$ \\ 
    ABL-OFS  & 20 & 5118 $m^2$ & 5425 $m^2$ \\ 
    ABL-VPBlocking  & 20 & 5118 $m^2$ & 5425 $m^2$ \\ 
...
\begin{table}[h!]
\centering
  \begin{tabular}{p{22mm} p{8mm} p{13mm} p{13mm} p{7mm}} 
 \hline
    Method & World & Mean $A$ & Max $A$ & Traveled \\ [0.5ex] 
 \hline\hline
    MonoSpheres  & Urban & 5118 $m^2$ & 5425 $m^2$ & 5 \\ 
    Grid-Based & Urban  & 1825 $m^2$ & 1981 $m^2$ & 5 \\
    % ABL-DistBased  & Urban & 5118 $m^2$ & 5425 $m^2$ & 5 \\ 
    % ABL-OFS  & Urban & 5118 $m^2$ & 5425 $m^2$ & 5 \\ 
    % ABL-VPBlocking  & Urban & 5118 $m^2$ & 5425 $m^2$ & 5 \\ 
    \hline
    MonoSpheres  & Cave & 5581 $m^2$ & 6012 $m^2$ & 3 \\ 
    Grid-Based & Cave  & 3119 $m^2$ & 3375 $m^2$ & 3 \\
    \hline
    MonoSpheres  & Rooftops & 5581 $m^2$ & 6012 $m^2$ & 3 \\ 
    Grid-Based & Rooftops  & 3119 $m^2$ & 3375 $m^2$ & 3 \\
    \hline
    MonoSpheres  & Sparse & 5581 $m^2$ & 6012 $m^2$ & 3 \\ 
    Grid-Based & Sparse  & 3119 $m^2$ & 3375 $m^2$ & 3 \\
    % OFS-disabled  & Cave & 4910 $m^2$ & 4910 $m^2$ & 0/1 \\ 
 \hline
\end{tabular}
...
We compare the grid-based approach with MonoSpheres in multiple experiments on 2 different worlds, and terminate each run after \SI{20}{\minute} or sooner if a collision occurs. The resulting maps and exploration progress are shown in \autoref{fig:exper_fireworld}, \autoref{fig:exper_cave} and the mean and max explored area in \autoref{table:sim_exper}.
...
Exploration goal generation was constrained to a 60x50x7 \SI{}{\meter} area of interest in the urban world for both methods, and to 170x110x20 \SI{}{\meter} in the cave world.
...
In the cave world with objects and walls rich in texture, neither of the compared methods collided with the environment.
Thus, we designed the urban environment to test the limits of our method, making it contain textureless areas (e.g. cars and burned parts of buildings) and thin obstacles such as exposed metal rods.
As expected, this led to collisions in both methods, as these objects cannot be detected by the used OpenVINS frontend.
...
This confirms the expected limitation of our method imposed by the used frontend, but also shows that the method can explore substantial areas even with this simple off-the-shelf sparse point-based SLAM frontend.
This limitation can be resolved in future work by designing a visual front-end that would would maintain the qualities of OpenVINS (mainly the runtime efficiency and precision), but would also allow tracking of both thin and large texture-sparse objects.
Alternatively, this issue could be solved by making the mapping pipeline multimodal and incorporating measurements from low-cost short-range distance sensors (e.g. ultrasonic).
...
In future work, our method could be improved by adopting a submap-based approach and a better-suited depth estimation frontend to achieve reliable monocular vision-based exploration at the same scale as LiDAR based systems.
