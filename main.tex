...
PolySphereMap - Occupancy Mapping using Sparse Monocular SLAM for Large-Scale 3D Exploration on an Inexpensive UAV}
...
\begin{abstract}
  ...
  \todo{
  We present a novel method of building a sphere-based occupancy representation using only a monocular camera along with pose estimates, which enables large-scale exploration and navigation on inexpensive UAVs. 
  ...
  We demonstrate how our method can be used to achieve, to the best of our knowledge, the first ever autonomous UAV exploration in large-scale outdoor 3D environments, using only a single onboard monocular camera and an IMU.
