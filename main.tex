...
Sphere-Based Occupancy Mapping for Fast and Safe Navigation of Vision-Based UAV Systems}
...
\begin{abstract}
  ...
  We open-source the code as a plug-and-play ROS module which constructs an occupancy map from RGB and odometry data suitable for path planning.
...
To move towards the reality of robust vision-based autonomous navigation of UAVs, our paper offers the following contributions:
\begin{itemize}
    \item A method of building an occupancy map using only sparse visual points and odometry as inputs
    \item A simple, rough inverse-depth estimator module as a frontend for the proposed occupancy mapping, designed to be used if only image and odometry data are available on a system (such as when the odometry is obtained from GPS). 
    \item Experimental demonstration that the proposed methods are suitable for online safety-aware navigation and exploration using UAVs equipped with only low-cost visual-inertial sensors.
...
The most notable is FLAME TODO-CITE, in which the authors track a large amount of visual keypoints, estimate their inverse depth using the prior pose knowledge, and interpolate these points into a mesh, which is the output of the module. 
The authors state that this method is highly dependent on the quality of the localization.
Another similar method is TODO-FIND-AND-WRITE.
...
TODO - standard is occupancy GRID. In many implementations - in 3D is most popular octomap

TODO - octomap - standard, but many problems. On larger distances, using raycast updating (lidar/rgbd), can have sparse map! BUT! Can just use one or few spheres!!!

TODO - voronoi, topological/topometric representations (usually built out of octomap!!)

TODO - spheregraph - map is known!!! (SEARCH!!!!)

TODO - spheremap - mention how shperes good! but this our work was imperfect cuz using octomap anyway!

TODO - we use spheres cuz good for planning, AND! we circumvent the problem of having to build octomap and then abstract from it!
AND! noone actually builds using SPARSE data!!! SO! we do that!
...
TODO - the quality of a representation is given only by its usefulness for some end tasks. and how much effort it takes to build it?

TODO - by navigation we mean all the things in those papers
...
% \section{Methodology}
% \section{Inverse-Depth Estimation Module}
% A great advantage of visual sensors over depth sensors such as RGB-D cameras, Time-Of-Flight Cameras or LiDARs, aside from their much lower cost, is the potential ability to estimate rough distances to very far away objects, which is present in most vision-based animals, but has not yet been well researched in robotics. 
% % RGB-D camera ranges are usually up to TODO meters, and LiDARs up to TODO meters.

% To utilize this advantage, we track keypoints in the RGB data and attempt to estimate their inverse-depth TODO-CITE, which is much better suited for larger distances than simple depth.

% Compared to this, ani
% In outdoor scenarios, for example on an open field, conventional depth sensors might often give no information about occupancy, as their range is usually up to 50m.
% It is possible to 
...
We propose a different, more lightweight approach to estimating free space over large distances, by constructing a polyhedron from the measured points and the robot, and inscribing spheres in that polyhedron, as shown in image TODO.
% Thus, even though raycasting might need tens of thousands of raycasts 

\subsection{Representation}

\subsection{From Sparse Visual Keypoints to Depth}
\subsection{Map Updating}

\subsection{Usability for Path Planning}
...
TODO - we use sampling?? Or connectivity?? DECIDE!!!
...
TODO - mention frontiers here???

TODO - mention strategy -- all frontiers near ground up to 50m away??

TODO - make MRS sim work again!!!
