...
in \cite{from_monoslam_to_explo}, the authors take points from semi-dense monocular SLAM and raycast towards them to build an OctoMap \cite{octomap} occupancy grid, but only demonstrate their local exploration approach building on top of this mapping pipeline to work inside a single room.
...
In this paper, we represent the free space using spheres as well, but with two major differences: 
...
\begin{itemize}
  ...
  \item In this paper, we build the graph of spheres \textit{directly} out of the sparse 3D pointclouds and pose estimates obtained from monocular SLAM, without needing to build an intermediate occupancy grid before computing the sphere radii, which frees up significant computational resources.
...
% TODO-CHECK
      \item The representation presented in this paper also contains obstacle points corresponding to triangulated visual keypoints with remembered measurement distance, which plays an important role when updating the map at different measurement distances (see Sec. \ref{sec:distance-based}). 
