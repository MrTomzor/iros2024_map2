...
% Selection_912_cam.png - VIO
  ...
  \caption{
    ...
    The mapping pipeline estimates depth using virtual (blue) and tracked (red) points from sparse monocular-inertial SLAM (left) running on board the UAV (top) to construct a large-scale map consisting of free-space spheres and obstacle points (right).
...
\begin{figure}[!htb]
  ...
  \caption{
    % The 4min orchard exploration experiment. The map (approx. 40x30m wide) contains noise due to low-light conditions, trees are highlighted in blue for clarity.
