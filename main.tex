...
Building an occupancy representation using a monocular camera with additional low-cost sensors is challenging for many reasons, and has so far been demonstrated only in small-scale, highly controlled environments, and has only allowed limited autonomy.
\todo{TODO-REF}
...
We further show how our mapping method enables autonomous exploration and navigation in large-scale, unstructured environments on a UAV using only a single camera and IMU as its sensors, which, to the authors' best knowledge, has not yet been achieved in literature at this scale.
