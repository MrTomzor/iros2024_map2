...
% where TODO.
% To introduce new spheres into the map, we sample a fixed number of points inside $\mathcal{P}_f$ at random distances between the camera and the depth mesh $\mathcal{F}_d$.
...
% The frontier updates, viewpoint sampling and long-distance planning runs on a separate thread also at approx. $\SI{5}{\hertz}$
% The most computationally intensive part, the frontier sampling, takes around TODO \%, while the other modules take significantly less time.
% A full re-implementation in C++ and a different way of computing exploration value than the current sampling approach could speed up the method further, but this is out of the scope of this paper.
}
...
\centering
  ...
  \caption{
    The 4min orchard exploration experiment. Right: explored map (approx. 40x30m wide), the UAV's trajectory (black arrows) and trees (circles blue).
  }
  \label{fig:orchard}
\end{figure}
% %%}
...
We have also successfully validated our approach, \fix{with the same parameters,} in a smaller, more cluttered orchard environment with thin tree branches, shown in \autoref{fig:orchard}.
...
\fix{For this reason, the volume metric is not fair towards methods using the grid-based raytracing mapping.}
...
Exploration goal generation was constrained by a 3D bounding box in the outdoor worlds for both methods.
...
.
% and did not estimate sufficient free space to 
...
We designed the earthquake environment to test the limits of our method, making it contain \fix{textureless surfaces} (e.g. smooth cars and burned parts of buildings) and thin obstacles such as exposed metal rods.
...
% Ablation experiments confirm the importance of the individual modules of our proposed method that were motivated by specific properties of monocular motion-based depth sensing.
