...
\begin{abstract}
  ...
  We present a novel spatial mapping approach that enables UAVs to explore large-scale 3D environments and requires only sparse pointclouds and pose estimates generated by monocular-inertial SLAM as input. Our method inscribes free-space spheres inside a polyhedron constructed around the camera's position and the visible SLAM points with low position covariance, instead of traditionally casting rays of free space towards the points to build an occupancy map. Intersecting spheres form a topometric map suitable for rapid safety-aware planning. We also present a scheme for safely sampling additional free space in textureless areas where depth estimation is challenging, and show how this enables navigation in outdoor open-space areas. Furthermore, we present a perception-aware exploration approach that is enabled by the proposed mapping method and explicitly handles the problematic properties of monocular depth estimation. We test the capabilities of our method in several real-world and simulated environments. To the best of the authors' knowledge, the proposed method is the first to achieve both indoor and outdoor 3D monocular exploration in unstructured environments up to 170x110m in scale. We open-source the code of the proposed method to provide safety-aware 3D navigation capabilities to any vision-based UAV operating in unknown environments.
...
The presented implementation of the proposed mapping and exploration methods is single-threaded and written in python.
With this implementation, the mapping runs at $\SI{2}{\hertz}$ on average on a standard CPU, which we found sufficient for real-time operation.
