...
\fix{MonoSpheres: Large-Scale Monocular SLAM-Based UAV Exploration through Perception-Coupled Mapping and Planning}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
  \fix{Autonomous exploration of unknown environments is a key capability for mobile robots, but it is largely unsolved for robots equipped with only a single monocular camera and no dense range sensors.
...
Code--- %\url{}
  \href{https://github.com/ctu-mrs/monospheres}{https://github.com/ctu-mrs/monospheres}
...
\fix{Monocular cameras and inertial measurement units (IMUs) are orders of magnitude lighter and less expensive, but monocular-only exploration of unknown environments has only been demonstrated at the scale of a few indoor rooms \cite{from_monoslam_to_explo, los_maps, simon2023mononav, cnn_explo_singleroom}.
...
\end{minipage}
  ...
  \caption{Illustration of the proposed approach. The mapping pipeline \cfix{estimates depth using virtual (blue) and tracked (red) points} from sparse monocular-inertial SLAM (left) running on board the UAV (top) to construct a large-scale map (right) consisting of free-space spheres (green), obstacle points (red) \cfix{and frontier points (magenta).}}
  \label{fig:smap_intro}
\end{figure}
% %%}


\fix{In this paper, we identify three core properties of motion-based depth sensing and show that explicitly designing the mapping and planning based on these principles can enable autonomy both indoor and outdoor, and at scales comparable to dense sensing methods.
...
\begin{itemize}
  \item \fix{A novel monocular 3D mapping method with perception-coupled modules that significantly increase the reliability of mapped free space and obstacles with unreliable, sparse, motion-based depth data as input.}
...
\section{\sectionfix{Problem Statement}}
\fix{
...
\fix{One approach to solving the monocular exploration problem is to use deep learning-based monocular depth estimation models to essentially turn the monocular camera into a dense depth sensor, and then use classical mapping and planning methods.
...
Compared to existing monocular exploration methods overall, our approach achieves exploration at larger scales, both indoor and outdoor, in unstructured 3D environments and without needing a GPU, thanks to the proposed perception-coupled mapping and planning modules.
}
...
\end{tikzpicture}
  \caption{
    \cfix{Mapping pipeline overview. The OVDE and DBOF modules are illustrated in \autoref{fig:ofs} and \autoref{fig:distbased} respectively.}
  }
...
\fix{The proposed mapping method represents free space by a graph $\mathcal{G}$ of intersecting spheres $\mathcal{S}$ and obstacles as a set of 3D points $\mathcal{X}_{map}$.}
...
\fix{To allow rapid path planning, we maintain a graph $\mathcal{G}$, which contains an edge for each pair of intersecting spheres.
...
\subsection{\sectionfix{Depth Interpolation and Polyhedron Construction}}
\label{sec:polyhedron}
\fix{We interpolate depth between the sparse SLAM points using a simplified version of the approach described in FLAME \cite{flame}, where
...
\begin{figure}[!h]
  % \includegraphics[width=0.5\textwidth]{fig/monospheres_ofs_v2.pdf}
      ...
      \end{tikzpicture}
  \centering
  \caption{\cfix{Open-area virtual-depth estimation diagram. The thick green points $\mathbf{x}_{vir, k}$ satisfy the condition defined in \autoref{sec:fake} and are added to the construction of $\mathcal{F}_d$}}
  \label{fig:ofs}
\end{figure}

% \subsection{Open-Area Free-Space Sampling (OFS)}
\subsection{\sectionfix{Open-Area Virtual-Depth Estimation (OVDE)}}
\label{sec:fake}
\fix{When moving in open areas, such as a grassy field without tall obstacles in \autoref{fig:smap_intro}, point-based visual SLAM can accurately localize only nearby points corresponding to the ground. 
...
% However, this could be resolved by using a dense SLAM frontend (at a higher computational cost), or by adding low-cost ultrasonic sensors to detect close thin obstacles.
% However, this could be resolved by adding low-cost ultrasonic sensors to detect close thin obstacles.
...
\begin{figure}[!h]
  ...
  The visible free-space polyhedron $\mathcal{P}_f^{t_2}$ is created using the \cfix{low-covariance SLAM points tracked at $t_2$} (black crosses) and the camera's \cfix{pose} $\mathbf{p}_{cam}^{t_2}$.
  The size of a point $\mathbf{x}$ \cfix{corresponds} to the distance $d_{\mathbf{x}}$ that it was measured from.
  \cfix{Since our method prioritizes closer measurements (of obstacles and lack thereof), only the (dashed red) point seen far away from the camera at a previous time $t_1$, which now lies in $\mathcal{P}_f^{t_2}$ very close to the camera, will be deleted as noise.}
  New spheres are inscribed inside $\mathcal{P}_f^{t_2}$, but their radii are also bound by the protected points (solid red) $\mathcal{X}_p$, which \cfix{were added to the map at $t_1$ and not deleted at $t_2$ even though they lie in $\mathcal{P}_f^{t_2}$.
...
\subsection{\sectionfix{Distance-Based Obstacle Filtering (DBOF)}}
\label{sec:distance-based}
\fix{As the next step of the update, we decide which tracked SLAM points to add into the map points $\mathcal{X}_{map}$, and which points to delete from the map.
...
\fix{The main issue addressed in this module is that, as shown in \autoref{fig:mapping_pipeline} and \autoref{fig:distbased}, points corresponding to a small obstacle might momentarily not be detected by the SLAM frontend, while a larger obstacle behind the small one is detected.}
Thus, we cannot simply delete every map point that falls into $\mathcal{P}_f$, because then the original points would be wrongly deleted, \fix{potentially causing collisions}.
...
\fix{Additionally, to prioritize more accurate depth measurements, points seen from a closer distance replace points seen from larger distances, if they are measured closer than $\xi$ to them.} 
...
\end{equation}
where $\mathbf{x}_{prev}$ is another mapped or newly added point with a higher position accuracy and \fix{$\xi$ is the obstacle map resolution.}
% \todo{TODO - rewrite as sort?}

\subsection{\sectionfix{Updating and Adding New Spheres}}
...
\fix{To introduce new spheres into the map, we sample a fixed number of points inside $\mathcal{P}_f$ in uniformly sampled directions at uniformly sampled distances between the camera and the depth mesh $\mathcal{F}_d$.}
...
{\section{\sectionfix{Perception-Coupled Exploration with a Sparse Sphere-Based Map}}
...
\fix{In this section, we describe the proposed perception-coupled navigation and exploration approach, visualized in \autoref{fig:exploration_explanation}.}
 ...
 \def\subfigwidth{0.48\linewidth}
  ...
  \subfloat{%
    ...
    \end{minipage}
     ...
     \hfill
  \caption{
    \cfix{Top-down illustration of the exploration approach.
...
\fix{\subsection{Perception-Aware Heading Control}}
\label{sec:align}
...
This way, the UAV reaches the viewpoint with at least $d_c$ meters of \fix{purely translational motion}.
...
\fix{When not aligning itself with a viewpoint as described above,
...
Another \fix{important} distinction we propose for NBV exploration with a monocular camera is to block sampling of new exploration viewpoints near visited viewpoints.
...
\fix{This is done because with motion-based depth sensing, no points will ever be measured on textureless surfaces.}
...
For this reason, we block the sampling of new viewpoints near visited viewpoints, \fix{visualized as black arrows in \autoref{fig:exploration_explanation} and \autoref{fig:exper_real}.}
...
The presented implementation of the proposed mapping and exploration methods \fix{is currently written in python in two threads.}
\fix{With this implementation, the mapping runs on average at $\SI{5}{\hertz}$ on 
% a standard CPU.
...
\centering
  ...
  \caption{
    \cfix{The 4min orchard exploration experiment. Right: explored map (approx. 40x30m wide), the UAV's trajectory (small black arrows) and trees (circled blue).}
  }
  \label{fig:orchard}
\end{figure}
% %%}
...
\fix{We have also successfully validated our approach, with the same parameters, in a smaller, more cluttered orchard environment with thin tree branches, shown in \autoref{fig:orchard}.}
...
\end{minipage}
  ...
  \centering
  \caption{\cfix{Trajectories of the best MonoSpheres (black) and Grid-Based explorer (blue) simulation experiments per each of the 4 worlds (top) along with the resulting MonoSpheres mapped obstacle points (bottom) colored by height. X marks the starting position.}
  }
...
\end{minipage}
  \centering
  \caption{\cfix{Ablation test results on the earthquake world. Left: visualization of exploration progress, best runs per method are thick and a marker signifies a crash or mission completion. Right: best-run trajectories for each method.}
  }
  \label{fig:ablations}
\end{figure}
...
% %%{
\begin{table}[h!]
  \caption{\cfix{Quantitative results showing the explored area in the simulation experiments described in \autoref{sec:sim_exper}.} }
\centering
...
\subsection{\sectionfix{Multi-World Simulation Evaluation}}
\label{sec:sim_exper}
...
\fix{In the following text, we show that it is the usage of this classical mapping method that limits exploration with sparse depth data in these previous works.
...
\fix{For this reason, the volume metric is not fair towards methods using the grid-based raytracing mapping and we focus more on explored \textit{area}.
...
\fix{Exploration goal generation was constrained by a 3D bounding box in each outdoor world for both methods.}

\subsection{\sectionfix{Simulation Results Discussion}}
\fix{
...
We designed the earthquake environment to test the limits of our method, making it contain textureless surfaces (e.g. smooth cars and burned parts of buildings) and thin obstacles such as exposed metal rods.
...
This is due to the inability of the depth estimation frontend to detect such objects, and could be improved by using dense SLAM frontend (at the cost of computational complexity), or by leveraging low-cost ultrasound sensors as a bumper for the UAV in future work.}
\label{sec:discussion}


\subsection{\sectionfix{Ablation Experiments}}
\label{sec:ablations}
\fix{To determine the contribution of the individual components of our proposed mapping and exploration method, we performed ablation tests in the earthquake world with 3 exploration runs for each ablation.
...
\section{CONCLUSION}
\fix{In this paper, we presented a novel, perception-coupled approach to mapping and planning for monocular UAV 3D exploration.
...
% our proposed approach achieves autonomous exploration at the scale of \fix{urban areas} with multiple buildings, trees and wide open spaces, both indoor and outdoor, in simulation and in the real world.
...
% scale of \fix{urban areas} with multiple buildings, trees and wide open spaces
