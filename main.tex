...
\todo{Under assumptions that hold in the majority of real-world environments (see \autoref{sec:fake}), this approach accurately maps free space even in low-texture areas where the traditional raycasting approach might not initialize enough free space to allow flight. } 
...
\todo{In several experiments, we show how this mapping approach, combined with perception-aware planning, enables exploration large-scale unstructured areas which feature both textureless wide open spaces and cluttered narrow passages.}
% TODO - is neccesary?
...
In these experiments, we showcase the capabilities of our approach in simulated  urban areas with multiple buildings, collapsed trees, cars and other obstacles.
...
The explored volume is measured by dividing the world into $\SI{1}{\meter}$ wide cells and summing the volume of cells that are intersected by at least one sphere.

First, we analyzed the exploration performance in a $\SI{110x70x20}{\meter}$ area over multiple 10 minute runs.
We compared the performance with the open-area free-space sampling (see \autoref{sec:fake}) both enabled and disabled.
The qualitative results can be seen in TODO and the resulting maps of the best runs are shown in \autoref{fig:exper_cranes}.
Without the additional free-space sampling, the UAV was only able to explore the mostly enclosed area with many obstacles and walls.
Only when our proposed novel open-space free-space estimation was enabled, the UAV could estimate sufficient free-space for exploration in the open-space area on the right with trees, cars and no enclosing buildings.
This experiment also shows that even when the more aggresive free-space sampling is enabled, our approach is still sufficiently safe to explore more space before running out of time or crashing.

Secondly, to test the scale limits of the method, we ran an experiment for 40 minutes in a $\SI{130x130x40}{\meter}$ simulated area.
The resulting map and original environment can be seen in \autoref{fig:exper_sim}.
The UAV managed to fully autonomously explore the environment while keeping the runtime of both the mapping and planning sufficiently low for real-time operation.
Additional experiment runs are available in the multimedia materials.

\subsection{Discussion}
In all the experiments, our method achieved collision-free flight even near complex obstacles such as trees, cranes and radio towers in uncommon configurations.
The few documented crashes were caused by either occasionally incorrect free-space sampling \autoref{sec:fake}, or by small featureless obstacles (such as the smooth metal pole near the fast-food building) that were not detected as features by point-based visual SLAM frontend.
Both of these issues could be solved in future work by using a visual front-end that uses line-based features, or by leveraging low-cost ultrasound sensors as a bumper for the UAV.
...
\begin{figure}[htb]
  ...
  \centering
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0.0\height}}, clip=true]{fig/explor_cranes_map.png}};
      \end{tikzpicture}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.02\width} {0.0\height} {0.0\width} {0.1\height}}, clip=true]{fig/explor_minusfreespace1a.png}};
      \end{tikzpicture}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{\subfigwidth}
      \begin{tikzpicture}
        \centering
        \node[anchor=south west,inner sep=0] (a) at (0,0) {\adjincludegraphics[width=1.0\linewidth, trim={{0.0\width} {0.0\height} {0.0\width} {0.1\height}}, clip=true]{fig/explor_plusfreespace1.png}};
      \end{tikzpicture}
     \end{subfigure}
     \hfill
  \centering
  \caption{Comparsion of the best resulting maps with the open-area free-space sampling disabled (middle) and enabled (bottom) in the smaller-scale exploration experiment.
  }
  \label{fig:exper_cranes}
\end{figure}
