...
\begin{figure*}[!htb]
  ...
  \centering
     ...
     \hfill
  \caption{
    Illustration of the mapping pipeline. TODO - desc + delaunay w fake fspace vis
  }
...
We estimate the currently visible free space as the volume between the camera's focal point $\mathbf{p}_{cam}$ and all points on $\mathbf{F}_d$.
...
However, we discard points that would fall into the Delaunay triangulation of the true obstacle points, since there we have a better depth estimate from the SLAM points.
...
\begin{figure}[!h]
  ...
  The visible free-space polyhedron $\mathbf{P}_f$ is created using the currently tracked SLAM points (black crosses) and the camera's position $\mathbf{p}_{cam}^{k}$ at update step $k$.
  ...
  The (dashed red) point seen from a long distance at a previous step $k-n$ is now not visible, and will be deleted as noise.
  The (dashed black) two points seen at a large distance now are not added to the map, as there are better localized (red) points near them in the map.
  New spheres are inscribed inside $\mathbf{P}_f$, but their radii are also bound by the protected points $\mathbf{X}_p$ (small red crosses), which were added to the map at $k-n$.
...
The points that lie in $\mathbf{P}_f$ and are not deleted are called \textit{protected points} $\mathbf{X}_p$.
These points are used to constrain sphere radii in the following update step.
Additionally, to prioritize closer measurements, points seen from a closer distance \todo{can replace} points seen at larger distances, if they are measured \todo{close enough to them}. 
...
To bound the update time of this step, we specify a maximum \todo{allowed} sphere radius $r_{max}$, which allows us to quickly filter out all spheres whose centers fall outside a bounding box around $\mathbf{P}_f$, inflated by $r_{max}$. 
...
Finally, we delete all spheres with $r_{k+1} < r_{min}$, where $r_{min}$ is the smallest \todo{allowed} sphere radius specified by the user. 
...
With rotation only, or motions that have too much rotation compared to translation, reliable depth estimates cannot be obtained \todo{using this method?}.

To solve this, when planning a path to a goal viewpoint that could uncover some frontiers, we need to compute the target headings on the path so that the UAV maintains a fixed heading in the viewpoint's direction for at least $d_{c}$ meters on the path before reaching the viewpoint, as visualized in Fig. \todo{TODO-FIG}.
...
Therefore, if there are \todo{visible visual keypoints} from that viewpoint, there will most likely be enough parallax to triangulate these points.
...
\todo{Another necessary distinction is that the UAV blocks sampling of new exploration viewpoints near visited viewpoints.
...
This approach is crucial for ensuring collision-free flight even in \todo{wrongly} initialized free space.
