...
\begin{enumerate}
  ...
  \item A novel 3D mapping approach, which constructs a sphere-based map using sparse pointclouds obtained from monocular SLAM as the only depth information.
    ...
    \item Open-sourced code for the novel mapping and exploration approach, along with example simulation scripts for running the entire pipeline.
\end{enumerate}
...
\label{sec:structure}
We propose to represent free space by a graph of intersecting spheres and surfaces as 3D points corresponding to triangulated visual keypoints from visual SLAM.
...
% We propose to represent free space by a graph of intersecting spheres and surfaces as 3D points corresponding to triangulated visual keypoints from visual SLAM.
% The main motivations for this representation instead of a traditional grid-based representation are twofold:


% First, as discussed in our previous work \cite{spheremap}, grid-based representation require a user-defined minimal voxel size.
% If this is set too large, tight corridors might seem obstructed in the map, and if set too small, updating the map can take too much time for real-time operation.
% Spheres, on the other hand, when pruned regularly (see \autoref{sec:graph_update}), cover space more efficiently and allow faster path-planning, even though the paths may be less smooth than from a fine grid.

% Secondly, grid-based representations are well suited when using dense range sensors, such as a LiDAR or depth camera.
% These sensors usually always detect an obstacle if it lies in the sensor's FOV, regardless of the sensor's motion.
% However, when using monocular SLAM points as obstacle measurements, during some motions an obstacle point might not be triangulated as a keypoint in the SLAM algorithm, and thus not visible.
...
\subsection{Obstacle Point Management and Motion-Based Free-Space Estimation for Open Areas}
\label{sec:open_space}
...
We have discussed and experimentally demonstrated how this new representation is specifically suited for 3D exploration and navigation in real-world outdoor conditions on a UAV equipped with only a single camera and IMU as the state estimation sensors.
