...
A grid-based representation, while being the basis for a substantial amount of robotic path planning and higher-level spatial abstraction methods, has several drawbacks for usage on UAVs equipped with monocular sensors in large-scale environments.
Firstly, an occupancy grid is updated by raycasting towards measured obstacle points.
This becomes problematic when the available measured points are very sparse, such as when using monocular SLAM.
Some works overcome this limitation to some degree -- in [MONO-EXPLO], the authors TODO and in [ROOM-DENSEDEPTH], the authors essentially perform dense pixel-based estimation with known poses from sparse SLAM, which works in well-illuminated rooms and may have problems in larger-scale or texture-poor environments.
In [ROOM-EXPLO], the authors deal with this by making the voxel size 0.5m, which might not capture narrow passages well, and only show the approach work in indoor environments.
...
In this paper, we represent the free space in the same way, but with the major difference that now \textit{we do not need to build the occupancy grid as an intermediate step}, which frees up significant computational resources.
...
\begin{enumerate}
  ...
  \item A novel occupancy mapping method which constructs a sphere-based occupancy representation using only sparse 3D point data from monocular visual feature tracking and odometry as input, easily extendable with dense point cloud data as well.
    ...
    \item An approach for monocular-inertial exploration on a UAV using the proposed occupancy mapping method and representation, along with important changes to traditional exploration approaches necessary for when using a monocular camera, experimentally verified in unstructured 3D environments on a UAV equipped with only a single camera and IMU as primary sensors, in simulation and in the real world.
    \item Open-sourced code for the map-building method, along with path-planning and exploration functionalities and example scripts to quick-start inexpensive UAV autonomy.
