...
\begin{abstract}
  ...
  % ------ VAR 2 - addition 
  % Using only a single camera and IMU, the method significantly reduces the cost of fully autonomous UAVs operating near obstacles, which have thus far relied on expensive and heavy distance sensors. 
  ...
  % Our method represents an environment by a graph of intersecting free-space spheres and a set of obstacle points instead of traditionally used grid-based maps. 
  % Our method represents an environment by a graph of intersecting free-space spheres and a set of obstacle points. 
  % To build this map, free-space spheres are inscribed inside a polyhedron constructed around the camera's position and currently tracked visual SLAM points at each iteration.
  Instead of traditionally casting rays of free space towards obstacle points to build an occupancy grid-based representation, our method inscribes free-space spheres inside a polyhedron constructed around the camera's position 
  % and currently tracked visual SLAM points at each iteration.
  and the currently visible triangulated visual keypoints at each iteration.
  Intersecting spheres then form a topological graph suitable for rapid and safety-aware planning.
  \todo{We also present a scheme for safely sampling additional free space in directions where no textured points can be triangulated, and show how this enables exploring outdoor open-space areas.}
  % We show how oversampling of free-space can allow flight through open-spaced outdoor areas.
  % Thus we build a topological graph of intersecting spheres, suitable for rapid and safety-aware planning, without needing any intermediate occupancy representation.
  % TOP
  % We also propose a method for safely estimating additional free space in large open outdoor areas with sufficient texture.
  % This allows navigating both cluttered and open spaces, where traditional sparse raycasting-based mapping approaches would not create enough free space for navigating.
  % TOP END
  % This map is built online, directly out of the sparse pointcloud measurements and requires no intermediate occupancy map unlike many topological mapping approaches.
  % Furthermore, we present a perception-aware exploration approach that is enabled by the proposed mapping method, and is tightly coupled to the properties of the monocular SLAM frontend.
  Furthermore, we present a perception-aware exploration approach that is enabled by the proposed mapping method
  % and is tightly coupled to the properties of the monocular SLAM frontend.
  % and explicitly handles the uncertainty brought by monocular depth estimation.
  % and explicitly handles the measurement errors of monocular depth estimation.
  and explicitly handles the problematic properties of monocular depth estimation.
The proposed methods are, to the authors' best knowledge, the first to achieve autonomous exploration in real-world, unknown, unstructured 3D environments at the presented scale, using only a single monocular camera and an IMU onboard a potentially micro-scale UAV.
  % We open-source the code for the proposed methods to allow any UAV equipped with a monocular camera and IMU to explore and navigate in unknown 3D environments.
  We open-source the code for the proposed methods to provide safety-aware 3D navigation capabilities to any vision-based UAV operating in unknown environments.
...
\todo{These advances could allow UAVs to be deployed for search-and-rescue in urban areas after natural disasters, monitoring wildfires, or mapping caves on other planets.}
...
Monocular exploration of unknown, unstructured 3D environments has only been presented at the scale of a few indoor rooms \cite{from_monoslam_to_explo, los_maps, simon2023mononav, cnn_explo_singleroom}.
Some works have presented outdoor monocular exploration, but designed only for highly specific types of environments (e.g. in corridors only \cite{corridors_mono_nav_2009} or around a single object with no other obstacles [\todo{TODO}]).
...
The existing monocular mapping approaches use the same core method which is used with dense range sensors:
...
Some methods \cite{from_monoslam_to_explo, los_maps} cast rays towards sparse pointclouds given by monocular SLAM, which leads to large gaps in free space, prohibiting planning.
Other methods \cite{mononnav} use depth neural networks to obtain denser pointclouds, 
...
The main idea of our paper is that instead of traditionally casting rays through a grid towards obstacle points and updating occupancy values of passed voxels \cite{octomap, voxblox}[\todo{MORE CITATIONS}], we construct a polyhedron of approximate visible free space at each frame and inscribe spheres of free space inside that polyhedron.
Intersecting spheres are connected together to form a topological graph suitable for rapid, safety-aware planning \cite{spheremap}.
\todo{Under a few assumptions that hold in the majority of real-world environments (see \autoref{sec:fake}), this approach accurately maps free space even in low-texture areas where the traditional raycasting approach might not initialize enough free space to allow flight. } 
% and yields a topological graph that allows rapid safety-aware planning.}
By storing obstacles as a set of geometric primitives (points), we can also assign additional metrics to each obstacle, such as measurement distance. 
...
\todo{In several experiments, we show how this mapping approach, combined with perception-aware planning, enables exploration in outdoor, large-scale areas which feature both textureless wide open spaces and cluttered narrow passages.}
% TODO - is neccesary?
...
\begin{enumerate}
  ...
  \item A novel 3D mapping approach that constructs a sphere-based map for safety-aware navigation using only sparse pointclouds and pose estimates from monocular-inertial SLAM as input,
    % with less conservative free-space estimation than traditional ray-casting methods.
    with the ability to map sufficient free space for flight even in open-spaced areas which lack reliably localizable visual features.
    % , without needing any intermediate occupancy map.
    % , mapping both open-space and cluttered areas.
  \item A perception-aware exploration approach, enabled by the proposed mapping method, that achieves \todo{large-scale} 3D outdoor exploration on a UAV equipped with only a monocular camera and IMU, 
      validated in real-world conditions.
    \item Open-sourced code for the proposed mapping and exploration methods, along with example simulation scripts for replicating \todo{and benchmarking} our results.
\end{enumerate}
...
Most of this research has been on reactive obstacle avoidance \cite{flame, avoidance_mono1} or exploration with strong assumptions on the environment structure \cite{corridors_mono_nav_2009}.
...
% For monocular exploration in unstructured environments, the authors of \cite{from_monoslam_to_explo, los_maps} proposed taking the points from monocular SLAM, treating them as sparse distance measurements, and building grid-based maps in the same way as with dense pointclouds.
For full monocular exploration in unstructured environments, the authors of \cite{from_monoslam_to_explo, los_maps} proposed building grid-based maps by casting rays of free space towards the monocular SLAM points. 
...
The authors of \cite{from_monoslam_to_explo} propose an exploration method specifically to cover these gaps.
...
The authors of \cite{los_maps} also cast rays towards sparse SLAM points, and present exploration on the scale of several indoor rooms and in simulation only.
They work around the gap issue by setting the map voxel size to be relatively large compared to the UAV size.
A larger voxel size causes a higher percentage of voxels to be passed by the sparse rays towards measured points, reducing the gaps, but leads to the following problem:
...
Grid-based maps built by raycasting suffer from the problem of choosing the smallest voxel resolution in multi-scale environments.
...
% \begin{itemize}
  Firstly, the methods presented in \cite{spheremap, spheregraph, bubbleplanner} require building a local occupancy grid with a LiDAR for computing the sphere-based map. 
    In this paper, we build the graph of spheres \textit{directly} out of input  pointclouds without building any intermediate occupancy grid. 
    Additionally, the proposed method requires only sparse pointclouds from a monocular camera running visual SLAM instead of dense LiDAR pointclouds.
...
To represent obstacles, we accumulate the stable (i.e. with low position covariance according to the SLAM) 3D points estimated by the monocular SLAM into a set of points $\mathcal{X}$, based on criteria described in \autoref{sec:distance-based}.
...
Consider a virtual point in space $\mathbf{x}_{vir}$ at time $t$ that not left the camera's FoV from a given time $t-x$ in the past up to $t$ 
...
\todo{This assumption allows estimating more free space than when only raycasting towards observed SLAM points.
...
However, it does not hold close to large textureless objects or thin objects where no features are detected by the visual SLAM.
...
In future work, this could be solved by adding low-cost, short-range distance sensors (e.g. an ultrasonic sensor), which are otherwise not usable for traditional dense raycast-based 3D mapping, or by using a denser depth estimation method.
}
...
We periodically check the described condition for a set of virtual points $\mathbf{x}_{vir}$ at a fixed distance in front of the UAV (see \autoref{fig:mapping_pipeline}) at each mapping iteration. 
...
However, we discard points that would fall into the Delaunay triangulation of the visible obstacle points, since there we have at least some depth estimate from interpolating the SLAM points.
...
\todo{This is designed to solve a problem that can arise when viewing some surface from a much higher distance than before (e.g. observing a wall from 5 m, and then later from 20m) ---
...
\begin{equation}
  \exists \mathbf{x}_{prev}: |\mathbf{x} - \mathbf{x}_{prev}| < k \quad \text{and} \quad d_{m,x_{prev}}  < d_{m,x}
\end{equation}
...
\todo{METRIC - NxN grid, covered when contains sphere! Can make simple script! (and can also output map as set of spheres and pts if needed! Useful for postproc! A weeks work to massively upgrade experiments!)}
...
As shown in Sec \ref{sec:runtimes}, even with this unoptimized implementation, the mapping keeps below $\SI{1.5}{\second}$ per map update on a standard CPU, making it suitable for real-world deployment.
...
The MRS UAV system \cite{mrs_uav_system} was used for trajectory generation and control.
...
In these experiments, we showcase the capabilities of our approach in a simulated $\SI{130x130x40}{\meter}$ urban area with multiple buildings, collapsed trees, cars and other obstacles.
...
\autoref{fig:exper_sim} shows the resulting map of a single example exploration mission.
Additional experiment runs are available in the multimedia materials.
The UAV managed to fully autonomously and with no prior information (except for the exploration bounding box so that the UAV does not explore far onto the open field) explore the vast majority of the environment.

The UAV managed collision-free flight even near complex obstacles such as trees, cranes and radio towers.
As described in \autoref{sec:fake}, small featureless obstacles (such as the smooth metal pole near the fast-food building) occasionally cause the UAV to crash, but this can be solved by a visual front-end that takes line-based features into account, or by leveraging low-cost ultrasound sensors as a bumper for the UAV.
Also, the explored area could be increased by implementing a more sophisticated exploration strategy, but even the simplistic greedy approach used in this paper proves the usefulness of our mapping approach.
...
\begin{figure}[!htb]
  ...
  \adjincludegraphics[width=9cm, trim={{0.05\width} {0.00\height} {0.05\width} {0.00\height}}, clip=true]{fig/runtimes_flood_almost_full.pdf}
  \centering
  \caption{Runtime analysis of individual steps of the map update explained in \autoref{sec:map_building}, recorded from the simulation experiment shown in \autoref{fig:exper_sim}.}
  % \caption{Runtime analysis of individual steps of the map update iterations, detailed in \autoref{sec:map_building}, recorded in the simulation experiment shown in \autoref{fig:exper_sim}.
  \label{fig:runtimes}
\end{figure}
...
% Thanks to only updating the map near the UAV, the update time remains relatively bounded.
Most of the mapping operations only take into account spheres and points in a bounding box around the visible free-space polyhedron (as more distant parts of the map cannot be affected by the polyhedron).
Thanks to the sparsification, the update complexity does not skyrocket, since the sphere population is bounded.
However, the overall runtime does slowly go up, most likely due to the polyhedron being large compared to the map size (the UAV travels at high speeds and triangulates distant points).
% However, in this experiment, the visible polyhedron is often quite large and encompasses most of the map, which.
We plan to overcome this in future work by separating the map into submaps, but the experiments demonstrate that a single map can be built at a reasonable rate for navigation even in a cluttered 130x130m area.
% The overall runtime grows with the map size as



\section{CONCLUSION}
In this paper, we proposed a novel approach to building a 3D map for navigation using a graph of spheres instead of a traditionally used voxel grid as the base representation, and presented how this mapping approach enables affordable UAVs to explore large-scale 3D environments. 
Our method only requires a monocular camera and an IMU, and can run on a standard onboard computer without needing a GPU.
Compared to the existing approaches to 3D monocular mapping and exploration, which have so far presented autonomy only at the scale of a few indoor rooms \cite{from_monoslam_to_explo, los_maps,deep_mono_depth_2021, deep_mono_depth_2019}, our proposed approach enables autonomous exploration at the scale of an urban area with multiple buildings, trees and wide open spaces, with all of the computations running on-board and online.


The proposed methods open the way for micro-scale UAVs operating fully autonomously in unknown indoor/outdoor large-scale environments with obstacles\todo{, but there are still many challenges to be solved.
...
Additionally, we plan to use the proposed mapping approach for building submaps instead of a single large map and performing large-scale map optimization combined with vision-based place recognition 
% to achieve low-cost autonomy at the scale of kilometers, which has only been demonstrated with LiDAR-based systems thus far.
to achieve low-cost autonomous exploration at the scale of kilometers, which has only been demonstrated with expensive LiDAR-based systems so far.
