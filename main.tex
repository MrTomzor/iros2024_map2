...
Some works have presented outdoor monocular exploration, but designed only for highly specific types of environments (e.g. in corridors only \cite{corridors_mono_nav_2009} or around a single object with no other obstacles \cite{information_driven_bbx_mapping}).
...
The main idea of our paper is that instead of traditionally casting rays through a grid towards obstacle points and updating occupancy values of passed voxels \cite{octomap, voxblox, ufomap}, we construct a polyhedron of approximate visible free space \todo{at each frame} and inscribe spheres of free space inside that polyhedron.
Intersecting spheres are connected together to form a graph-of-spheres map representation that has been shown to allow rapid and safety-aware planning \cite{topomap, skeletons, spheremap}.
