...
We propose to represent free space by a graph of intersecting spheres, similarly to our previous work \cite{spheremap} and obstacles by points corresponding to triangulated visual keypoints from monocular SLAM (or, in the future, input points from dense range sensors).
...
In the experiments in this paper, we build on top of OpenVINS, using its mode of estimating inverse depth estimates.

Estimating inverse depth of visual keypoints [INVERSE] bring considerable advantages occupancy mapping compared to estimating keypoint 3D positions, as this method of estimation allows the UAV to know that there are points in nearly infinite distance in some direction.
We do not add such distant points to the map, but we can use them to estimate that up to some distance, there is free space in that direction, such as when a UAV sees a tree line in the distance, or flies between skyscrapers.
There can be object that are too small to be detected by the visual SLAM, so the free-space estimates might be wrong, but we deal with that as well, in Sec \ref{sec:distance-based}.
