...
\fix{The main result of the simulation experiments is that our approach is capable of exploring many different topologies of environments -- sparse, dense, indoor, outdoor and environments where flight over obstacles in wide open spaces is required.
...
In all of the outdoor worlds, the classical mapping approach with random exploration goal sampling was only able to map the area close to tall obstacles near the starting position.}
...
\section{\sectionfix{Discussion and Limitations}}
...
We designed the Earthquake environment in \autoref{sec:sim_exper}
 to test the limits of MonoSpheres with the current frontend, making it contain textureless surfaces and thin obstacles.
...
This could be solved in future work by integrating low-cost but noisy short-range ultrasonic sensors into the mapping pipeline to act as a bumper, 
...
or by using a dense SLAM frontend (at the cost of computational complexity).
