...
To explore, the method periodically samples free-space points uniformly in a 40x40x10 \SI{}{\meter} area around the robot and sends these as goals to a grid-based A* path planning module, which finds path through the occupancy grid with the same minimum distance from obstacles and uknown space as the MonoSpheres method (\SI{1.5}{\meter} in these experiments).
The robot then follows these paths with the camera pointing forward and the planner replans at \SI{5}{\hertz} to handle measurement inacurracies.}
...
or by using dense a SLAM frontend (at the cost of computational complexity).
