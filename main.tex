...
However, in \cite{spheremap}, we built the SphereMap incrementally out of an OctoMap.
In this paper, we present a method to build the SphereMap \textit{directly out of sparse 3D point data from a monocular camera} and \textit{without needing to construct an occupancy grid}, thus enabling rapid safety-aware path planning for resource-constrained UAVs without costly range sensors.
