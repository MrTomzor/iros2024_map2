...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
  % TODO - cely prepsat, nova iterace!
  ...
  % and demonstrate how our method enables safe exploration and navigation in unstructured environments on inexpensive UAVs.
  Our main contribution is representing environments by a graph of free-space spheres and a set of triangulated visual keypoints with remembered measurement uncertainty, which is fundamentally different from grid-based environment maps. 
  % We present a way of building this representation in real-time using only sparse 3D point data obtained from sparse monocular SLAM onboard inexpensive UAVs.
  ...
  % We describe an exploration and navigation system that takes advantage of the proposed representation, and demonstrate 
  We describe an exploration system that uses the advantages of the proposed representation to overcome some of the challenges of monocular vision-based UAV autonomy, and demonstrate 
large-scale autonomous UAV exploration in unstructured indoor/outdoor 3D environments, using only a single onboard monocular camera and an IMU, in simulation and in the real world.
  % , which enables large-scale exploration and navigation on inexpensive UAVs. 
  ...
  % Our approach consists of two main parts -- first, we obtain sparse visual keypoints with inverse depth estimates using monocular or monocular-inertial SLAM.
  % Depth is interpolated between visible keypoints at any keyframe, which is a large simplification that can miss objects not seen by the sparse visual SLAM, but we propose methods to deal with this problem in a robust manner.
  % We then estimate measured free space at any keyframe by forming a polygon between the camera and the visible keypoints, and inscribe spheres inside it, which are then used for fast safety-aware path planning.
  % Visual points visible at some keyframe 
  % Then, from thus obtained sparse point clouds, we estimate visible free space by a polyhedron, and inscribe spheres inside it, which are then connected to a graph that is used for efficient and safety-aware path planning.
  ...
  % We demonstrate how our method can be used to achieve, to the best of the authors' knowledge, the first repeatable autonomous UAV exploration in unstructured outdoor 3D environments, using only a single onboard monocular camera and an IMU.
% [TODO - o experimentech]
  ...
  We open-source the code for our approach to provide occupancy mapping and navigation to any UAV with a single camera and any pose estimation.
...
In addition, cameras are useful for object recognition or other types of vision-based artificial intelligence, and thus 
...
We further show how our mapping method enables autonomous exploration and navigation in large-scale, unstructured environments on a UAV using only a single camera and IMU as its sensors, which, to the authors' best knowledge, has not yet been achieved in literature.
...
A grid-based representation, while being the basis for a substantial amount of robotic path planning \todo{[SOME PLANNING]} and higher-level spatial abstraction methods \todo{[VOXGRAPH, SKELETONS, SPHEMAP, TOPOMAP]}, has several drawbacks for usage on UAVs without dense range sensors in large-scale environments.
...
Some works overcome this limitation to some degree -- in \todo{[MONO-EXPLO]}, the authors \todo{TODO and in [ROOM-DENSEDEPTH]}, the authors essentially perform dense pixel-based estimation with known poses from sparse SLAM, which works in well-illuminated rooms but may have problems in larger-scale or texture-poor environments.
In \todo{[ROOM-EXPLO]}, the authors deal with this by making the voxel size 0.5m, which might not capture narrow passages well, and they only show the approach work in indoor environments.
...
In our previous work \cite{spheremap}, we have shown how a graph of intersecting spheres can efficiently represent both large-scale and narrow space, and that this representation allows path planning that has explicit information about distance to obstacles, in addition to allowing orders of magnitude faster path planning than on an occupancy grid.
In this paper, we represent the free space using spheres as well, but with \todo{two major differences 
...
With only a monocular camera however, building a reliable map for exploration and navigation is even more challenging, as \todo{obtaining correct depth estimates is harder than with stereo cameras.}
...
Compared to the above mentioned approaches, we demonstrate exploration and navigation in 3D using a monocular camera and an IMU, with \todo{no assumptions} on the environmental structure, and in large-scale environments, with all of the computations running on-board the UAV without requiring a GPU.
...
\begin{enumerate}
  ...
  % \item A novel occupancy mapping approach, which constructs a sphere-based occupancy representation that offers the same information as an occupancy grid, but is more suitable for path planning, using odometry and sparse pointclouds obtained from monocular SLAM.
    ...
    \item A demonstration of large-scale 3D outdoor exploration on a UAV equipped with only a monocular camera and IMU as its sensors, enabled by the proposed occupancy mapping method, 
...
\section{PolySphereMap for Safety-Aware Monocular-Inertial 3D Exploration}
...
\begin{figure}[!htb]
  ...
  \caption{The proposed map (bottom) after 20min of exploring an urban environment (top) in simulation, explained in more detail in TODO-REF. Black arrows indicate visited frontier viewpoints, frontiers are shown as purple points.}
  \label{fig:exper_sim}
\end{figure}
