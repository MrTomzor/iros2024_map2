...
In \autoref{fig:exper_real}, we present a 20min experiment where our approach is validated in real-world conditions.
...
As the previously published monocular exploration methods do not have their code published (with the exception of \cite{simon2023mononav}, which, however, is not ROS-compatible as of writing this paper), we prepared a simple grid-based mapping and exploration pipeline for comparison.
...
The robot then follows these paths with the camera pointing forward and replans at 5Hz to handle measurement inacurracies. 
...
However, because of the open-space free-space sampling scheme and the depth interpolation, MonoSpheres constructs considerably more free space than the raytracing mapping approach over the same trajectories. 
...
Exploration goal generation was constrained to a \SI{60x50x7}{meter} area of interest in the urban world for both methods, and was unconstrained in the enclosed cave world.
...
Another possibility of detecting these objects would be to use a visual front-end that uses line-based or dense features, but that is beyond the scope of this article.
...
In future work, our method could be improved by adopting a submap-based approach and a better-suited depth estimation frontend, to achieve reliable exploration at the same scale as LiDAR based systems.
